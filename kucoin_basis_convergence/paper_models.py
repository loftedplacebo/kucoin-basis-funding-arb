...
from kucoin_basis_convergence.models import format_datetime, parse_datetime, parse_float, parse_int, utc_now


@dataclass
class ConvergencePaperPosition:
    position_id: str
    base: str
    direction: str
    spot_symbol: str
    ...
    estimated_net_pnl_ex_funding_usd: float
    ...
    @classmethod
    def from_csv_row(cls, row: dict) -> "ConvergencePaperPosition":
        return cls(
            ...
            entry_zscore=parse_float(row.get("entry_zscore"), 0.0) or 0.0,
            current_zscore=parse_float(row.get("current_zscore"), 0.0) or 0.0,
            entry_percentile=parse_float(row.get("entry_percentile"), 0.0) or 0.0,
            current_percentile=parse_float(row.get("current_percentile"), 0.0) or 0.0,
            target_basis_pct=parse_float(row.get("target_basis_pct"), 0.0) or 0.0,
            ...
            estimated_net_pnl_ex_funding_usd=parse_float(
                row.get("estimated_net_pnl_ex_funding_usd"),
                0.0,
            )
            or 0.0,
