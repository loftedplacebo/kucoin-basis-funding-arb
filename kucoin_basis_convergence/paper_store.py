...
from kucoin_basis_convergence.models import format_datetime, parse_datetime, utc_now
...
POSITION_FIELDS = [
    ...
    "estimated_net_pnl_ex_funding_usd",
...
class ConvergencePaperStore:
    def __init__(self, config: KucoinBasisConvergenceConfig):
        self.config = config
    ...
    def load_all_positions(self) -> list[ConvergencePaperPosition]:
        if not self.positions_path.exists():
            return []
        with self.positions_path.open("r", newline="", encoding="utf-8") as f:
            return [ConvergencePaperPosition.from_csv_row(row) for row in csv.DictReader(f)]

    def load_open_positions(self) -> dict[str, ConvergencePaperPosition]:
        return {
    ...
    def write_positions(self, positions: dict[str, ConvergencePaperPosition]) -> None:
        closed = [
            position
            for position in self.load_all_positions()
